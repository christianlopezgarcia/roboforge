...
class PCA9685PWM:
    def __init__(self, i2c, freq_hz=50):
        ...
        freq_hz sets the PWM frequency (default = 50 Hz for motors/servos).
        """
    ...
    def set_raw(self, ch: int, duty16: int):
        """
        Set raw 16-bit PWM duty cycle on a channel.
        duty16 range: 0–65535
        """
    ...
    def set_percent(self, ch: int, percent: float):
        """
        Set duty cycle using percentage (0–100%).
        Automatically converts to 16-bit scale.
        """
...
class myMotors():
    ...
    # Pin assignments for PCA9685 each motor gets it's assignment and the next pin
    pinAssignments = {"FR":12,"FL":10,"BR":14,"BL":8}
    
    # Array for reversing the direction of the motors
    motor_rev_arr = {"FR":1,"FL":1,"BR":1,"BL":1}
    
    # Lower limit in percent of motor power (to prevent ringing) (currently not used)
    ...
    def __init__(self,myIMUobj,myPWMobj,angle_pid):
        """
        Initializes motor controller with IMU feedback and PWM driver.
        Loads PID gains for steering control.
        """
        self.myIMU = myIMUobj
        self.myPWM = myPWMobj        
    ...
    def set_pid_enable(self,enable):
        """
        Enable or disable the steering PID.
        """
        self.enable_pid = enable
    
    def set_desired_angle(self,angle):
        """
        Sets the target heading angle and clears integral term.
        """
    ...
    def move(self,direction):
        """
        Sets target forward speed based on direction command.
        """
        if(direction == "FWD"):
    ...
    def move(self,direction = "FWD", duration = 0.1):
        """
        Moves the motors forward or backward for a duration in seconds and then stops.
        """
    ...
    def turn(self,angle):
        """
        Adds a relative angle change to the current heading.
        """
        self.set_desired_angle(self.current_angle + angle)
    
    def turn_and_move(self,direction,duration):
        """
        Turns the robot right or left, moves forward for a duration,
        then turns back to the original heading.
        """
        # Stores past set values
        buf_p = self.angle_p
        buf_i = self.angle_i
        ...
        # Turn 90 degrees in desired direction
        self.turn(dir*90)
        now = time.time()
        ...
        # Move forward for a duration in seconds
        time.sleep(0.1)
        self.move("FWD",duration)
        time.sleep(0.1)
    ...
    def set_single_motor(self,motor,percent):
        """
        Drives one motor forward or backward using two PWM pins.
    ...
    def kill_motors(self):
        """
        Kills output to motors and resets current commands and PID I buffer.
        """
    ...
    def limit_pid_component(self, value, limit):
        """
        Clamps a PID term to a safe limit.
        """
    ...
    def limit_angle_pid(self):
        """
        Clamps P and I buffers to certain limits.
        """
    ...
    def limit_motor_value(self,value):
        """
        Limits the input motor value to a lower limit.
        Currently bypassed
        """
    ...
    def angle_wrap(self,angle):
        """
        Wraps angle error between -180 and 180.
        """
        return (angle + 180.0) % 360.0 - 180.0    

    def update_angle_pid(self):
        """
        Updates the steering PID based on heading error.
        """
        # Get error and wrap the angle between -180 and 180        
        angle_error = self.desired_angle - self.current_angle
        angle_error_wrapped = self.angle_wrap(angle_error)
        
        #Get current time and time since last pid loop
        now = time.time()
    ...
    def update_motor_output_tank(self):
        """
        Applies speed + steering PID output to left/right motors.
        """
        # Set speed component 0-50%
        motor_speed_value = self.desired_speed

        # Set angle component 0-50%
        ...
        # Compute motors values between 0-100%
        ...
        # Limit motor value from going below lower limit
    ...
    def update_motors(self):
        """
        Updates the motor power values and PIDs.
        """
        # Check if the enable parameter has been changed and kill motors if so
    ...
    def none_check(self,old_data,new_data):
        """
        Checks if incoming data is "None" and sets output to the previous data if so.
        """
        if(new_data is None):
            print("None Error")
            return old_data
    ...
    def print_state_info(self):
        """
        Prints current angle and desired angle. 
        """
        print("--------------State Info------------------")
    ...
    def print_pid_info(self):
        """
        Prints current PID buffer values and associated motor power.
        """
        print("--------------PID Info------------------")
...
if __name__ == "__main__":
    ...
    # Uncomment to test turn and move functionality
    # motors.turn_and_move("Right", 5)

    while 1:
        
        # Uncomment to test spin functionality
        # if(time.time() - time_last_update > update_time):
        #     motors.turn(20)
        #     motors.update_motors()
        #     time_last_update = time.time()
        
        # Uncomment to test turning functionality
        # if(time.time() - time_last_turn > 5)
        #   motors.turn(90)
        #   time_last_turn = time.time()
        # if(time.time() - time_last_update > update_time):
        #     motors.update_motors()
        ...
        # Break out of loop at max_runtime
